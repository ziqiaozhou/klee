...
klee_alias_function("block_signals","faked_block_signals");
	cur=(struct thread_info *)malloc(sizeof(struct thread_info));
...
#define symbol_define(tk,field,type,name) type field;klee_make_symbolic(&field,sizeof(type),name);
	skb->next=NULL;
	skb->prev=NULL;
	...
	tk->retransmit_skb_hint=NULL;
	 ...
	 u64     bytes_received;
	 klee_make_symbolic(&bytes_received,sizeof(bytes_received),"tk->bytes_received");
	 u32     rcv_nxt;
	 ...
	 u32     lsndtime;  
	 klee_make_symbolic(&lsndtime,sizeof(lsndtime),"tk->lsndtime");
	 tk->lsndtime=lsndtime;
	 tk->rcv_tstamp=rcv_tstamp;
	 u32     tsoffset;
		 tk->tsoffset=tsoffset;
...
void init_net2(void){
	...
	(&init_net)->mib.tcp_statistics=malloc(sizeof(struct tcp_mib));
	int tcp_size=sizeof(struct tcp_mib);
	memcpy((&init_net)->mib.tcp_statistics,tcp_mibs,tcp_size);
	struct linux_mib* net_statistics=malloc(sizeof(struct linux_mib));
	(&init_net)->mib.net_statistics=malloc(sizeof(struct linux_mib));
	klee_make_symbolic(net_statistics,sizeof(struct linux_mib),"net_statistics");
