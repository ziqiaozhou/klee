...
void faked_inet_put_port(struct sock *sk){
}
void faked_reqsk_fastopen_remove(struct sock *sk, struct request_sock *req,
			bool reset)
{

}
static void faked_inet_csk_reset_xmit_timer(struct sock *sk, const int what,
			unsigned long when,
			const unsigned long max_when)
{
	...
	if (what == ICSK_TIME_RETRANS || what == ICSK_TIME_PROBE0 ||
		...
		icsk->icsk_retransmit_timer.expires= icsk->icsk_timeout;
	} else if (what == ICSK_TIME_DACK) {
		icsk->icsk_ack.pending |= ICSK_ACK_TIMER;
		icsk->icsk_ack.timeout = jiffies + when;
		icsk->icsk_delack_timer.expires= icsk->icsk_ack.timeout;
		}

	}
	bool faked_tcp_pause_early_retransmit(struct sock *sk, int flag){
		struct tcp_sock *tp = tcp_sk(sk);
		unsigned long delay;
...
#define FLAG_ECE                0x40
		...
		if (!time_after(inet_csk(sk)->icsk_timeout, (jiffies + delay)))
		  return false;
		faked_inet_csk_reset_xmit_timer(sk, ICSK_TIME_EARLY_RETRANS, delay,TCP_RTO_MAX);
		return true;
	}
	void faked_tcp_mark_head_lost(struct sock *sk, int packets, int mark_head)
	{
		struct tcp_sock *tp = tcp_sk(sk);
		struct sk_buff *skb;
		int cnt, oldcnt;
	...
	void faked___tcp_v4_send_check(struct sk_buff *skb, __be32 saddr, __be32 daddr){
	do{}while(0);
	}
	void faked___tcp_push_pending_frames(struct sock *sk, unsigned int cur_mss,  int nonagle){
	}
	void faked_tcp_send_ack(struct sock *sk){
	}

	unsigned int faked_tcp_current_mss(struct sock *sk){
		return 1<<((sizeof(unsigned int)*8)-1);
	}

	void faked_tcp_check_space(struct sock *sk){

	}
	void faked_sk_reset_timer(struct sock *sk, struct timer_list* timer,
				unsigned long expires){
	}
	int faked_tcp_sendmsg(struct kiocb *iocb, struct sock *sk, struct msghdr *msg,
				size_t size)
	{
		return 0;
	}
	static ssize_t faked_do_tcp_sendpages(struct sock *sk, struct page *page, int offset,
	...
	static random_id=0;
		void faked_prandom_bytes(void *buf, size_t nbytes){
			...
			char name[]="prandom0";
			name[7]+=random_id;
			klee_make_symbolic(buf,nbytes,name);
		}
	...
	void faked_tcp_store_ts_recent(struct tcp_sock *tp){
	}
...
#define MAXRESERVED 10
	...
	/*	unsigned int faked_tcp_current_mss(struct sock *sk){
		...
		//	klee_alias_function("tcp_current_mss","faked_tcp_current_mss");
			...
			klee_alias_function("tcp_store_ts_recent","faked_tcp_store_ts_recent");
			klee_alias_function("sk_reset_timer","faked_sk_reset_timer");
			...
			klee_alias_function("__tcp_push_pending_frames","faked___tcp_push_pending_frames");
	...
	void init_sock2(struct sock* sk){
		...
		sk->sk_wq       =       NULL;
		sk->sk_prot=&tcp_prot;
		sk->sk_prot->unhash=&faked_unhash;
		sk->sk_cgrp=NULL;
...
#define SYMBOL_DEFINE_SNMP_STAT(type,name) \
	...
	type name,name ## _2;\
	klee_make_symbolic(&(name ## _2),sizeof(__typeof__(type)),"output_init_net.mib."#name);\
...
#define PRINT_atomic_MIB(type,name)\
	...
	PRINT_atomic_MIB(struct icmpmsg_mib, icmpmsg_statistics)
