...
class Parser:
    ...
    def changeKey(self,one):
        one=one.replace("3794294743",str(random.randint(0,4294967295)))
        return one;
    ...
    def testKleaver(self,path,count):
        ...
        outpath=path.replace('.pc1','.pc-new')
        alltime={}
        ValidTime={}
        InValidTime={}
        maxWidth=int(math.pow(2,8))
        for i in range(0,int(count)):
            for width in range(1,maxWidth,maxWidth/32):
                allline=[]
                low=random.randint(0,maxwidth-width)
                pair=[low,low+width]
                for line in allline0:
                    line=line.replace("UPPERBOUND",str(max(pair))).replace("LOWERBOUND",str(min(pair)))
                ...
                of.close()
                start = time.clock()
                result=subprocess.check_output("kleaver --use-cache=0 -use-cex-cache=0 "+outpath,shell=True)
                end=time.clock()
                result=result.split('\n')
                check=result[0].split();
                if check[2]=='VALID':
...
if len(sys.argv)>1:
    ...
    parse.testKleaver(sys.argv[1],sys.argv[2])
    #parse.parseResult()
    #parse.parseAll()
